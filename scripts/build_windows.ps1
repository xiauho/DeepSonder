...
$SidecarDistRoot = Join-Path $ProjectRoot "generated\sidecar"
...
if ($ElectronPackage.version -ne $Version) {
    throw "VERSION ($Version) and package.json ($($ElectronPackage.version)) differ."
}
...
Invoke-Checked "Build Python Sidecar" {
    & $PythonExecutable -m PyInstaller --clean --noconfirm `
        ...
        (Join-Path $ProjectRoot "deepsonder_electron_sidecar.spec")
}
$SidecarExecutable = Join-Path $SidecarDistRoot "DeepSonderElectronSidecar.exe"
...
$UnpackedExecutable = Join-Path $ReleaseRoot "win-unpacked\DeepSonder-Electron.exe"
$UnpackedSidecar = Join-Path $ReleaseRoot "win-unpacked\resources\sidecar\DeepSonderElectronSidecar.exe"
foreach ($required in @(
    $UnpackedExecutable,
    $UnpackedSidecar,
...
$Installer = @(Get-ChildItem -LiteralPath $ReleaseRoot -File -Filter "DeepSonder-Electron-v$Version-windows-x64-setup.exe")
$PortableZip = @(Get-ChildItem -LiteralPath $ReleaseRoot -File -Filter "DeepSonder-Electron-v$Version-windows-x64.zip")
...
if (-not (Test-Path -LiteralPath $RecoveryExecutable -PathType Leaf)) {
    throw "The portable recovery ZIP does not contain DeepSonder-Electron.exe at its root."
}
...
$Manifest = [ordered]@{
    ...
    entrypoint = "DeepSonder-Electron.exe"
    sidecar = "resources/sidecar/DeepSonderElectronSidecar.exe"
    artifacts = $Artifacts
