...
function assertCandidateGate(report) {
  ...
  if (report.authenticode !== "passed" &&
    ...
    throw new Error("候选包的 Authenticode 状态不符合发布级别。 ");
  }
  if (report.gates?.authenticode !== report.authenticode) {
    throw new Error("候选包的 Authenticode 证据不一致。 ");
  }
}
...
export function buildReleaseReadiness(candidateGate, aiGate) {
  ...
  return {
    ...
    release_tier: candidateGate.release_tier,
    ...
    evidence: {
      windows_clients: ["windows-10", "windows-11"],
      metadata_signature: "passed",
      authenticode: candidateGate.authenticode,
