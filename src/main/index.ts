...
const repositoryRoot = electronRoot;
...
if (selfTestMode) {
  ...
  app.setPath(
    "userData",
    path.join(app.getPath("temp"), `deepsonder-electron-self-test-${process.pid}`),
  );
}
...
function resolveSidecarCommand(): ConstructorParameters<typeof SidecarClient>[0] {
  const selfTestRoot = path.join(
    app.getPath("temp"),
    `deepsonder-sidecar-self-test-${process.pid}`,
  );
  ...
  if (app.isPackaged) {
    return {
      command: path.join(process.resourcesPath, "sidecar", "DeepSonderElectronSidecar.exe"),
      args: [],
