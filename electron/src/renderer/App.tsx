...
import type {
  ...
  ManuscriptItem, ManuscriptSnapshot, OpenedProjectV2, PreferencesSnapshot, ProjectSnapshot,
...
export function App() {
  ...
  const startAI = async (kind: AITaskKind) => {
    if (aiRunning) return;
    if (kind !== "connection" && (project === null || document === null || currentChapter === undefined)) {
    ...
    const result = await window.novalist.startAITask({
      kind,
      chapterId: kind === "connection" ? "" : currentChapter!.id,
  ...
  return <main className={`app-shell theme-${preferences.theme}`} style={{ fontSize: `${preferences.uiFontSize}px` }}>
    <header className="topbar">
      <div className="brand-lockup"><div className="brand-mark">N</div><div><strong>Novalist</strong><span>Electron</span></div></div>
    ...
    <section className="workspace">
      <aside className="rail" aria-label="主导航">
        ...
        <button className={aiOpen ? "rail-item active" : "rail-item"} onClick={() => void showAI()} disabled={project === null}><span>✦</span>AI</button>
        ...
        <button className="rail-item trash-rail" onClick={() => void showTrash()} disabled={project === null}><span>♲</span>回收站</button>
      </aside>
      <aside className="document-panel">
        <div className="panel-heading"><div><span className="eyebrow">{projectV2 !== null ? "MANUSCRIPT V2" : "PROJECT LIBRARY"}</span><h2>{projectV2 !== null ? "正文目录" : "创作资料"}</h2></div>{project !== null && <button className="icon-button" title="新建资料" onClick={() => openCreate("chapter")} disabled={busy}>＋</button>}</div>
        {projectV2 !== null && manuscript !== null ? <div className="library-body">
          <div className="library-tools"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索章节…" aria-label="搜索正文章节" /><p className="v2-library-note">人物与世界观不会沿用旧资料，将在后续识别流程中重新建立。</p></div>
    ...
    {trashOpen && <div className="modal-layer" role="presentation"><section className="trash-card"><header><div><span className="eyebrow">PROJECT RECYCLE BIN</span><h2>项目回收站</h2></div><button className="icon-button" onClick={() => setTrashOpen(false)}>×</button></header><p>删除的资料保存在当前项目中。永久删除后不可恢复。</p><div className="trash-list">{trash.items.length === 0 ? <div className="trash-empty">回收站是空的</div> : trash.items.map((item) => <article key={item.trashId}><div><strong>{item.title}</strong><span>{KIND_LABEL[item.kind]} · {formatDeletedAt(item.deletedAt)}</span><small>{item.originalPath}</small></div><button onClick={() => void restoreTrash(item)} disabled={busy}>恢复</button><button className="danger-button" onClick={() => void deleteForever(item)} disabled={busy}>永久删除</button></article>)}</div></section></div>}
    {aiOpen && <aside className="ai-drawer" aria-label="AI 任务面板">
      ...
      <p className="ai-scope">任务只读取完成当前工作所需的项目资料；生成结果不会自动写入。</p>
      <div className="ai-actions"><button onClick={() => void startAI("expand")} disabled={aiRunning || currentChapter === undefined}>扩写正文</button><button onClick={() => void startAI("continuation")} disabled={aiRunning || currentChapter === undefined}>续写章节</button><button onClick={() => void startAI("check")} disabled={aiRunning || currentChapter === undefined}>一致性检查</button><button onClick={() => void startAI("memory")} disabled={aiRunning || currentChapter === undefined}>生成记忆提案</button><button onClick={() => void startAI("connection")} disabled={aiRunning}>测试 DSH</button></div>
