...
class PromptBuilderTests(TestCase):
    ...
    def test_write_prompt_contains_task_contract_and_context(self) -> None:
        system, user = prompt_texts(
            prompt_builder.build_write_prompt(self.project, "chapter_01", 2000)
        )
    ...
    def test_expansion_prompt_uses_outline_without_current_body(self) -> None:
        system, user = prompt_texts(
            prompt_builder.build_expansion_prompt(self.project, "chapter_01", 2000)
        )
    ...
    def test_style_guide_is_applied_only_to_novel_generation_prompts(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            _system, check_user = prompt_texts(
                prompt_builder.build_check_prompt(project, "chapter_01")
            )
    ...
    def test_expansion_prompt_contains_only_selected_foreshadowing(self) -> None:
        ...
        review_system, review_user = prompt_texts(
            prompt_builder.build_foreshadowing_review_prompt(
                "chapter_01",
                "古剑铭文揭示了铸剑者。",
                selected,
            )
    ...
    def test_prompt_history_window_is_configurable(self) -> None:
        ...
        _system, continuation_user = prompt_texts(
            prompt_builder.build_write_prompt(
    ...
    def test_expansion_retry_prompt_explicitly_corrects_agent_preamble(self) -> None:
        system, user = prompt_texts(
            prompt_builder.build_expansion_retry_prompt(
                self.project, "chapter_01", 2000
            )
        )
    ...
    def test_check_prompt_requires_structured_report(self) -> None:
        system, user = prompt_texts(
            prompt_builder.build_check_prompt(self.project, "chapter_01")
        )
    ...
    def test_repair_prompt_is_minimal_and_anchored(self) -> None:
        system, user = prompt_texts(
            prompt_builder.build_consistency_repair_prompt(
                self.project,
                "chapter_01",
                {
                    "issue_id": "issue_1",
