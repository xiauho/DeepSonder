...
- **沉浸式写作台**：蓝白浅色主题（可切换蓝黑深色）、统一一级导航、可收起侧栏、专注模式、Markdown 源码/只读预览切换、查找替换和未保存状态提示
- **写作统计**：实时显示字数、段落、预计阅读时长和光标位置
- **安全保存**：定时自动保存，切换资料时自动落盘，退出前检查未保存内容
- **故事资料库**：管理大纲、章节、项目写作风格、角色、世界观、战力体系和时间线
- **角色档案同步**：按连续章节范围，从已采用且版本有效的故事记忆中提取角色状态变更；逐字段确认后仅更新角色卡受管区域，并自动备份同步前版本
...
读取能力，并要求返回文件头、正文中部和文件尾三个随机校验值；首次回执异常时会复用
同一任务文件和校验值自动重试一次，重试后仍无有效回执或文件能力不可用时任务会安全
停止，不会把业务提示词降级到命令行。任务文件在成功、失败、
...
配置 schema v4 增加 AI 创作前文参考模式。新配置默认自动参考：兼容优先最近 3 章、
...
配置 schema v5 增加 AI 创作的“补充更早相关剧情”开关，默认启用；自定义参考 0 章时，
...
同样使用可追溯历史选择器；设置页的章数同时用于扩写与续写，远期参考开关控制扩写、
续写和一致性检查。
修复、记忆更新不增加远期摘要输入。

配置 schema v6 将原“AI 扩写字数”统一为“目标章节字数”，默认 3000 字。该设置表示包含
已有正文在内的整章目标；Novalist 统一按“排除空白、包含标点”的正文字符数统计。扩写与
续写以目标的 95%～105% 为理想范围、90%～110% 为审阅范围；任务启动后固定本次目标
快照，首轮与格式纠偏共用同一字数契约。低于审阅范围的结果先自动进行一次完整篇幅纠偏；
仍低于理想范围且自动修正预算尚有余量时，再使用由应用生成的段落位置编号进行局部差额
补写。模型只选择编号并返回插入正文，不再复制原文锚点；单个无效插入项不会阻止其他安全、
独立的插入项应用。格式纠偏、完整篇幅纠偏和自动补写共用最多两次自动修正机会。原稿会在
完整纠偏成功后保留供切换，预览按轮次显示处理结果；仍然偏短时提供“重新补写”和“仍然
采用”。明显超出审阅范围时采用前需要再次确认，超长正文不会自动截断。续写以目标字数减去
当前正文有效字数作为参考，单次请求最多 3000 字；正文为空、已达到目标或剩余不足 300 字时
不会启动续写。
...
记忆建议使用 V2 模型响应协议，只生成 `digest.summary` 一份权威章节摘要，并仅回传
短请求 ID、结构化摘要、状态变更建议和冲突候选。章节、正文、旧状态和设定哈希由应用
本地绑定，不再要求模型复制；Patch 的旧值与写入前置条件也由本地状态计算。新任务只
接受 V2 响应，旧模型响应属于临时任务结果，不再兼容；已有项目记忆和已采用记录不受
影响，无需重新生成。可信提案缓存使用独立格式，不保存重复的顶层摘要。
...
记忆更新统一使用事实账本/Patch 流程：先从分块事实归并章节摘要，再生成带事实依据的
白名单状态变更建议；应用根据任务启动时的旧状态生成 `expected_before` 前置条件，并在
本地检查字段碰撞、过期状态、
...
2. 从左侧打开章节，在中间编辑器中写作；可使用编辑器右上角的“源码 / 预览”切换查看 Markdown 渲染效果，预览模式只读且不会改变或自动保存源码。
3. 使用“AI 创作”中的扩写或续写、“检查设定”或“更新记忆”时，首次调用会显示数据处理告知；调用期间可在右侧“上下文”页查看实际传输方式、预算占用和各类资料的保留或裁剪情况；检查报告页可直接选择其他章节运行检查。
4. 一致性问题只有在正文原句能够唯一定位且适合安全替换时才开放“AI 修复”；修复结果必须经过预览确认，且不会自动保存章节。
5. 需要让角色卡跟进剧情时，先逐章采用故事记忆，再从“创作 → 同步角色档案”或角色右键菜单选择角色与连续章节范围；推断性变更默认不勾选。
6. 审核 AI 输出后再保存、采用或公开。
7. 通过“文件 → 导出全书”生成 `.md` 或 `.txt` 文件。
...
每次 AI 调用还会在右侧“上下文”页生成一份仅驻留于当前应用会话的脱敏报告，记录任务类型、字符预算、实际传输方式、各上下文分区的原始长度与发送长度、裁剪状态以及临时文件清理结果。报告还会按资料类别显示候选、明确相关、预选、最终纳入、未匹配排除、分类容量排除和总预算排除数量，并列出“全局核心”“手动选择”“标题命中”等选择原因。扩写纠偏重试、差额补写、伏笔复核、事实提取和记忆 Patch 等多次调用会分别列出。若 Harness 未按正文协议返回标记而触发纯正文兼容回退，AI 输出区会明确显示本次次数和当前应用会话累计次数。报告与诊断不包含章节正文、设定原文、完整提示词、凭据、资料文件名或绝对路径。
