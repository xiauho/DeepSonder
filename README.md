...
每次 AI 调用还会在右侧“上下文”页生成一份仅驻留于当前应用会话的脱敏报告，记录任务类型、字符预算、实际传输方式、各上下文分区的原始长度与发送长度、裁剪状态以及临时文件清理结果。扩写纠偏重试、伏笔复核、章节摘要和故事状态更新等多次调用会分别列出。报告不包含章节正文、设定原文、完整提示词、凭据或文件绝对路径；复制诊断信息时同样只复制这些统计字段。
