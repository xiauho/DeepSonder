...
`ai_context_selection_mode` 默认为 `safe`。该模式只使用本地、确定性的标题、
Markdown 小标题、别名、标签和角色名称匹配，不会为筛选额外调用模型或联网；
相关条目优先进入分类容量，未匹配资料作为低优先级背景。全局核心规则、标记为
核心的体系和作者本次手动选择的体系不参与普通背景竞争。如果兼容旧项目时需要
恢复原有整体加载顺序，可在配置文件中临时改为 `legacy_all`。
...
每次 AI 调用还会在右侧“上下文”页生成一份仅驻留于当前应用会话的脱敏报告，记录任务类型、字符预算、实际传输方式、各上下文分区的原始长度与发送长度、裁剪状态以及临时文件清理结果。报告还会按资料类别显示候选、明确相关、预选、最终纳入、未匹配排除、分类容量排除和总预算排除数量，并列出“全局核心”“手动选择”“标题命中”等选择原因。扩写纠偏重试、伏笔复核、章节摘要和故事状态更新等多次调用会分别列出。报告不包含章节正文、设定原文、完整提示词、凭据、资料文件名或绝对路径；复制诊断信息时同样只复制这些统计字段。
