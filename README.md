...
配置 schema v6 将原“AI 扩写字数”统一为“目标章节字数”，默认 3000 字。该设置是扩写
提示词、长度校验和差额补写的唯一目标来源；任务启动后固定本次目标快照，不受运行期间
设置变化影响。首次扩写低于动态允许范围时，应用会自动请求一次仅含插入项的差额补写，
验证唯一原文锚点后再本地合并，不允许模型删除或替换首次正文。补写仍未达标时由作者在
预览中决定是否采用，超长正文只提示而不会截断。续写以目标字数减去当前正文有效字数
作为参考，单次请求最多 3000 字；正文为空、已达到目标或剩余不足 300 字时不会启动续写。
